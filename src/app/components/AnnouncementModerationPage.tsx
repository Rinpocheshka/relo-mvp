    ...
    <div className="min-h-screen bg-background pb-16">
      ...
      <div className="bg-white border-b border-border/40 sticky top-0 z-10 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-5 flex items-center justify-between">
          ...
          <span className="text-xs bg-terracotta-deep/10 text-terracotta-deep px-3 py-1.5 rounded-full font-semibold">
      ...
      <div className="max-w-5xl mx-auto px-4 pt-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-muted-foreground">
            ...
            <p className="font-medium">Объявлений нет</p>
          </div>
        ) : (
          <div className="space-y-4">
            <AnimatePresence mode="popLayout">
              {announcements.map((ann) => {
                ...
                const statusInfo = STATUS_LABELS[ann.status] ?? { label: ann.status, color: 'bg-gray-100 text-gray-600' };
                return (
                  <motion.div
                    key={ann.id}
                    layout
                    ...
                    className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${
                      ann.status === 'pending' ? 'border-amber-300/60' : 'border-border/60'
                    }`}
                  >
                    {/* Card header */}
                    <div className="p-5">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <h3 className="font-bold text-foreground truncate">{ann.title}</h3>
                          <div className="flex flex-wrap gap-x-3 gap-y-1 mt-1 text-xs text-muted-foreground">
                            <span className="flex items-center gap-1"><User className="w-3 h-3" />{ann.author_name}</span>
                            <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{ann.city}</span>
                            {ann.price_text && <span className="flex items-center gap-1"><Tag className="w-3 h-3" />{ann.price_text}</span>}
                            <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{new Date(ann.created_at).toLocaleDateString('ru-RU')}</span>
                          </div>
                        </div>
                      ...
                      {ann.status === 'pending' && (
                        <div className="flex gap-2 mt-4">
                          <Button
                            size="sm"
                            onClick={() => setStatus(ann.id, 'active')}
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => setStatus(ann.id, 'rejected')}
                            disabled={actionLoading === ann.id}
                          ...
                          <div className="p-5 space-y-4">
                            {ann.description && (
                              <div>
                                ...
                                <p className="text-sm text-foreground whitespace-pre-wrap leading-relaxed">{ann.description}</p>
                              </div>
                            )}
                            {ann.images?.length > 0 && (
                              <div>
